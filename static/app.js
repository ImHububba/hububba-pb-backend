...
function createPeerConnection(peerSid) {
  ...
  pc.onicecandidate = (e) => {
    if (e.candidate) {
      socket.emit("signal", { to: peerSid, from: mySid, type: "candidate", payload: e.candidate });
    }
  };
...
async function handleOffer(fromSid, offer) {
  ...
  socket.emit("signal", { to: fromSid, from: mySid, type: "answer", payload: answer });
}
...
screenBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    newTrack.addEventListener("ended", async () => {
      ...
      if (current) { localStream.removeTrack(current); current.stop(); }
...
socket.on("connect", () => { mySid = socket.id; console.log("connected as", mySid); });
